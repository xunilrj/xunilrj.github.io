...
async function loadUnload(oldValue, newValue, load, oldUnload, newUnload, set, token, setProgress) {
    ...
    if (token.cancel) return;
    set({ unload: newUnload, state: "loaded", value: xx, firstTime: false });
}

export default function useAsyncLoadUnloadState(initialValue, load, unload) {
    const [v, set] = useState({ unload, firstTime: true });

    let token = { cancel: false };
    let setProgress = (x, str) => {
        set({ unload, state: "loading", progress: x, message: str, firstTime: false });
    }
    let setNewValue = (newValue) => {
        token.cancel = true;
        set({ unload, state: "loading", firstTime: false });
        token = { cancel: false };
    ...
    if (v.firstTime) {
        load(initialValue, setProgress).then((x) => {
            if (token.cancel) return;
            set({ unload, state: "loaded", value: x, firstTime: false });
        });
