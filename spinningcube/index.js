...
function renderTargets(canvas, gl) {
  ...
  const o = {
    ...
    gen: (gl2, width, height, depth) => {
      let depthBuffer;
      if (depth) {
        depthBuffer = gl2.createRenderbuffer();
        gl2.bindRenderbuffer(gl2.RENDERBUFFER, depthBuffer);
        gl2.renderbufferStorage(gl2.RENDERBUFFER, gl2.DEPTH_COMPONENT16, width, height);
      }
      const targetTexture = gl2.createTexture();
      gl2.bindTexture(gl2.TEXTURE_2D, targetTexture);
      {
        const level = 0;
        const internalFormat = gl2.RGBA;
        const border = 0;
        const format = gl2.RGBA;
        const type = gl2.UNSIGNED_BYTE;
        const data = null;
        gl2.texImage2D(gl2.TEXTURE_2D, level, internalFormat, width, height, border, format, type, data);
...
async function buildShader(gl, vertCode, fragCode) {
  ...
  for (let i = 0; i < qtd; ++i) {
    let info2 = gl.getActiveUniform(program, i);
    let index = gl.getUniformLocation(program, info2.name);
    if (index) {
      uniforms.push({
        i: index,
        info: info2
      });
      uniformByName[info2.name] = index;
    }
  }
  var uniformBlocks = [];
  ...
  return {
    ...
    setUniform: (name, v) => {
      ...
      let info2 = gl.getActiveUniform(program, i);
      if (info2.type == gl.FLOAT_MAT4)
        gl.uniformMatrix4fv(i, false, v);
      if (info2.type == gl.FLOAT_VEC3)
        gl.uniform3fv(i, v);
    }
  };
}
...
function buildUBO(gl, data) {
  ...
  gl.bindBufferBase(gl.UNIFORM_BUFFER, bindi, buffer2);
  gl.bindBuffer(gl.UNIFORM_BUFFER, null);
  return {
    buffer: buffer2,
    bindi,
...
const fdrawCube = function(gl, cr, cg, cb, ca, shaderProgram, options, globalUB, proj_matrix, view_matrix, model_matrix, cubevao) {
  ...
  if (options && options.glowColor) {
    ...
    gl.uniform3fv(index, new Float32Array([options.glowColor.r / 255, options.glowColor.g / 255, options.glowColor.b / 255]));
  }
...
async function initRotatingCube(canvas, gl, renderTargets2) {
  ...
  bindVertexArrayAndShader(gl, cubevao, normalRender, {});
  var proj_matrix = mat4.create();
  var view_matrix = mat4.create();
  var model_matrix = mat4.create();
  const sceneTarget = renderTargets2.gen(gl, 256, 256, true);
  const hBlurTarget = renderTargets2.gen(gl, 256, 256, false);
  const vBlurTarget = renderTargets2.gen(gl, 256, 256, false);
  const fullSceneTarget = renderTargets2.gen(gl, canvas.clientWidth, canvas.clientHeight, true);
  ...
  let fdraw = function(time) {
    ...
    mat4.lookAt(view_matrix, vec3.fromValues(-3, 3, 3), vec3.fromValues(0, 0, 0), vec3.fromValues(0, 1, 0));
    let glowColor = {
      r: 255,
      g: 0,
      b: 0
    ...
    const smallScene = renderTargets2.renderTo((x) => fdrawCube(gl, 0, 0, 0, 0, glowRender, {
      glowColor
    }, null, proj_matrix, view_matrix, model_matrix, cubevao), sceneTarget);
    const hBlurTexture = renderTargets2.renderTo((x) => {
      hBlurRender(smallScene, ([t, blurStepSize]) => {
        gl.uniform1f(t, (Math.cos(screent) + 1) / 2);
        gl.uniform1f(blurStepSize, 1 / 256);
      });
    }, hBlurTarget);
    const bluredTexture = renderTargets2.renderTo((x) => {
      vBlurRender(hBlurTexture, ([t, blurStepSize]) => {
        gl.uniform1f(t, (Math.cos(screent) + 1) / 2);
        gl.uniform1f(blurStepSize, 1 / 256);
      });
    }, vBlurTarget);
    const fullSceneTexture = renderTargets2.renderTo((x) => fdrawCube(gl, 0.5, 0.5, 0.5, 1, normalRender, null, null, proj_matrix, view_matrix, model_matrix, cubevao), fullSceneTarget);
    renderTargets2.renderToScreen(() => mixTexturesRender([fullSceneTexture, bluredTexture]));
  };
  return fdraw;
}
...
function useColorPicker() {
  var el = useRef();
  useEffect(() => {
    colorPicker = new iro2.ColorPicker(el.current, {
      width: 150,
      color: "#f00"
    });
  }, []);
